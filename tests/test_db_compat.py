...
def test_timezone_roundtrip_is_aware_on_postgres(engine):
    """
...
def test_aware_minus_readback_datetime_matches_backend(engine):
    """
    ...
    async def scenario(db):
        ...
        await db.commit()
        db.expire_all()
        return (await db.execute(select(Order).where(Order.id == "o-sub"))).scalar_one().paid_at
...
def test_overlong_string_is_rejected_only_on_postgres(engine):
    """
    VARCHAR(n) 的执行力度。
    ...
    切到 PG 才开始 500。上线前必须确认这些字段有截断或足够长度。
    """
    ...
    if is_postgres():
        ...
        _run(engine, scenario)   # SQLite 静默接受

        async def check(db):
            row = (await db.execute(select(Store).where(Store.id == "s-len"))).scalar_one()
            return len(row.platform_account)

        assert _run(engine, check) == 300, "SQLite 应原样存下超长字符串（不截断）"
...
def test_json_column_roundtrip(engine):
...
def test_unique_violation_raises_integrity_error(engine):
    """
...
def test_boolean_roundtrip(engine):
...
def test_float_price_precision(engine):
    ...
    async def scenario(db):
        ...
        await db.commit()
        db.expire_all()
        return (await db.execute(select(Order).where(Order.id == "o-price"))).scalar_one().amount

    assert _run(engine, scenario) == 128.88
...
def test_string_comparison_is_consistent(engine):
    """
...
def test_like_is_case_sensitive_on_postgres(engine):
    """
    LIKE 的大小写敏感性。
